...
Current phase: Phase 0/1/2/3/4 merged to main and tagged (`v2.0.0-alpha`, `v2.0.1-beta`, `v2.0.2-beta`, `v2.0.3-beta`); Phase 5 (FreeRTOS control task) implemented, awaiting review - none of Phase 0-5's exit conditions are bench-verified yet - see docs/BASELINE.md, docs/PHASE1_NOTES.md, docs/PHASE2_NOTES.md, docs/PHASE3_NOTES.md, docs/PHASE4_NOTES.md and docs/PHASE5_NOTES.md for the outstanding bench checklists  
...
- [x] Start with a 10 ms period using `vTaskDelayUntil()`. `CONTROL_TASK_PERIOD_TICKS = pdMS_TO_TICKS(10)`.
...
- [x] Choose task priority, core affinity and stack size deliberately; document the reason. Core 1 (same as `loopTask`, not the Wi-Fi/BT driver's core), priority 5 (above `loopTask`'s 1, below the radio stack's ~23), 4096-byte stack. See [PHASE5_NOTES.md](PHASE5_NOTES.md) §2.
...
- [x] Feed the Task WDT with `esp_task_wdt_reset()` only after a complete successful control cycle, including safety evaluation and actuator/deadman refresh. Called immediately after `controlStep()` returns, never before.
- [x] Check and handle the return values of the Task WDT API calls. Both `esp_task_wdt_add()` and `esp_task_wdt_reset()` are checked and logged on failure.
- [ ] Measure worst observed cycle time and deadline misses. Instrumented (`controlTaskWorstCycleUs`/`controlTaskDeadlineMisses`, Serial report every 5s) but not bench-measured - no hardware. See [PHASE5_NOTES.md](PHASE5_NOTES.md) §4 and its bench checklist.
- [ ] Measure stack high-water mark. Instrumented (`uxTaskGetStackHighWaterMark()`, same Serial report) but not bench-measured.
...
- [ ] Stress Wi-Fi, web requests, SD access and OTA preparation during a simulated/bench shot. Not bench-tested.

Exit condition: control timing remains deterministic under service-side load, the control task itself is watched by the Task WDT, and no deadline or stack problems are observed. **Partially met**: the mechanism (dedicated task, higher priority than `loopTask`, explicit watchdog subscription) is implemented and reasoned through in [PHASE5_NOTES.md](PHASE5_NOTES.md), but the actual timing/stack/watchdog-recovery numbers this exit condition asks for require bench hardware not available in this environment. This phase also surfaced a new correctness question - real task concurrency now exists for the first time, and a handful of globals Phase 4 §4 flagged as "worth re-examining once Phase 5 makes these genuinely concurrent" (`setpoint`/`steamSetpoint`/PID tunings/`offset`) are re-examined, not yet resolved, in [PHASE5_NOTES.md](PHASE5_NOTES.md) "Considered, not changed."
...
| 2026-08-23 | Phase 5 | `phase5-control-task`, see PR | Started immediately after the Phase 4 merge/tag above, on explicit direction. Moved `controlStep()` into a new dedicated task (`controlTaskEntry()`, created last in `setup()` via `xTaskCreatePinnedToCore()`), running a fixed 10 ms period with `vTaskDelayUntil()`; `controlStep()`'s own body did not need to change - `GetPressure()`/`runPID()` already self-gated on `PRESS_INTERVAL`/`PID_INTERVAL` via their own `millis()` checks, so this really was the relocation Phase 4 set up for, not a redesign. Chose and documented core 1 (same as `loopTask`, not the Wi-Fi/BT driver's core) at priority 5 (above `loopTask`'s 1, below the radio stack's), and a 4096-byte stack. Explicitly subscribed the task to the Task WDT with `esp_task_wdt_add(NULL)`, feeding it with `esp_task_wdt_reset()` only immediately after `controlStep()` returns; both calls' return values are checked and logged on failure. Added Serial-only diagnostics (worst cycle time, deadline-miss count, stack high-water mark, reported every 5s) so the bench checklist's timing/stack items are measurable once hardware is available, without adding anything to the HTTP/telemetry surface. `loop()` no longer calls `controlStep()` directly. While implementing, re-examined the Phase 4 §4 "worth re-examining once Phase 5 makes these genuinely concurrent" note and found it real: `setpoint`/`steamSetpoint` (both `double`, non-atomic on this hardware) and `myPID.SetTunings()` are now written from `loopTask` (`/saveConfig`) while read/used from the genuinely concurrent `controlTask` (`runPID()`'s `myPID.Compute()`) for the first time - a real torn-read/torn-internal-state risk that didn't exist before this phase, not yet fixed, written down as a named target for review. Compiles cleanly: 907,529 bytes flash (+796 vs `v2.0.3-beta`), 51,308 bytes RAM (+16). Full detail in `PHASE5_NOTES.md`. | Independent model review of this PR - in particular the self-identified `setpoint`/`steamSetpoint`/PID-tunings concurrency question - then bench-test everything still outstanding from Phase 0-5 before Phase 6 |
