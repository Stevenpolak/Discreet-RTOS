...
Current phase: All 8 phases merged to main and tagged (`v2.0.0-alpha` through `v2.0.7-beta`) - the staged rewrite is complete at the code level. None of Phase 0-8's exit conditions are bench-verified yet - see docs/HARDWARE_TEST_PROCEDURE.md for the single, ordered, prioritized runbook covering everything outstanding across every phase (starts with Phase 7's two highest-priority items: the pressure sensor's fault-signaling convention, and whether a genuine over-temperature condition is reachable in practice). docs/ARCHITECTURE.md is the reference for the resulting task/queue/timing design. Phase 9 (physical power/sleep/steam controls, see below) is net-new functionality, not yet started, deliberately sequenced after this point so it builds on a documented base.  
...
| 2026-08-25 | Phase 8 | `phase8-cleanup-and-handoff`, see PR | Started on explicit direction ("please hit the next phase"). Removed two dead globals (`lastPrintTime`, `brewTemp`) after auditing every actuator write site for duplication (none found - Phase 3/6 already fully centralized ownership). Added `docs/ARCHITECTURE.md` (task boundaries, queue schemas, timing) and `docs/HARDWARE_TEST_PROCEDURE.md` (every phase's bench items consolidated into one ordered runbook). Updated the root README and this checklist, and added an upstream-contribution recommendation (several small PRs against `Discreet-Coffee/Discreet`, not one large one) - a recommendation only, no PR opened. Compiles cleanly: 909,165 bytes flash (-180 vs `v2.0.6-beta`), 51,324 bytes RAM (-16). Full detail in `PHASE8_NOTES.md`. | Independent review of this pass, then apply fixes |
| 2026-08-25 | Phase 8 review fixes | `phase8-cleanup-and-handoff`, see PR | 4-agent independent review found two more dead pieces of state the first draft's own "obsolete globals" pass had missed - `TelemetrySnapshot::heaterOn` (write-only since Phase 7 switched `handleGetValues()` to recompute it directly; left in place it was an active trap for the next snapshot consumer) and the `PIDonly` global (duplicated `currentMode`; its one runtime write was never read by anything, and the comment justifying it was checked against `/saveConfig` and found factually wrong) - both fixed. Also found and fixed a substantial set of accuracy/completeness problems across the two new docs: several imprecise or self-contradicting claims in `ARCHITECTURE.md` (missed the buzzer's cross-task mutex; a "one writer" claim contradicted by its own table; a timing-table row that inverted Gap 5's own finding) and a missing "State model" section; six bench items from Phase 5/6's own notes silently dropped from `HARDWARE_TEST_PROCEDURE.md`'s consolidation, plus one step that read as contradicting Gap 2 (the pump-deadman gap) instead of testing it. The upstream-contribution recommendation was checked with `git apply --check` against the real `Discreet-Coffee/Discreet` tree (not just read against this repo's own history) and found to have two real mistakes - Phase 3 proposed before Phase 2 despite depending on Phase 2's enums, and one of the "portable" Phase 7 fixes actually depending on this rewrite's own infrastructure - both corrected. Compiles cleanly: 909,097 bytes flash (-68 vs the pre-fix commit), 51,324 bytes RAM (unchanged). Full detail in `PHASE8_NOTES.md` "Review round 1." | Get sign-off on the fixes, merge and tag |
| 2026-08-25 | Phase 8 tagged, merged | `main`, tag `v2.0.7-beta` | PR merged to `main` after the review fixes above; tagged and published as a pre-release. This completes all 8 phases of the staged rewrite plan at the code level. None of Phase 0-8's exit conditions are bench-verified - `docs/HARDWARE_TEST_PROCEDURE.md` is the ordered, prioritized runbook for closing that out, starting with the pressure-sensor fault convention and over-temperature reachability. | Bench-test per `HARDWARE_TEST_PROCEDURE.md`, or proceed with Phase 9 (physical power/sleep/steam controls) if directed |
